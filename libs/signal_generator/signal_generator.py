...
class SignalGenerator:
    def __init__(self):
        ...
        for signal_name in self.signals_names:
            self.signals[signal_name] = {'x': Queue(max_size=self.window_size) , 'y':Queue(max_size=self.window_size)}

    def measure_signals(self):
        time_stamp=self.time_stamps.__next__()
        for signal_name in self.signals_names:
            self.signals[signal_name]['x'].enqueue(time_stamp) if signal_name == 'sin' else self.signals[signal_name]['x'].enqueue(time_stamp)
            self.signals[signal_name]['y'].enqueue(np.sin(time_stamp)) if signal_name=='sin' else self.signals[signal_name]['y'].enqueue(np.cos(time_stamp))
