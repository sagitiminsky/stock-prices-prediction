...
stock_names=apps.config.stock_names if apps.config.sin==False else libs.signal_generator.config.signals_names
...
stocks_obj=GetStocksInfo() if apps.config.sin==False else SignalGenerator()

dl_models=DLModels(window_size=window_size,config=config)
callback=CallBack()


for i in tqdm(range(apps.config.window_size+10**2)):
    stocks_obj.measure()
    batch = stocks_obj.stocks[stock_names[0]]['queueObj']._norm_queue if apps.config.sin==False else stocks_obj.stocks[stock_names[0]]['y']._queue
    # print(len(batch))
    if len(batch)==window_size:
        ...
        for stock_name in stock_names:
            ...
            if trainX_s is None:
                ...
                if apps.config.prediction_type == 'MANY2MANY':
