...
for i in tqdm(range(config.window_size + 10 ** 3)):
    ...
    stock_monitor = signals_obj.signals[config.signal_names[0]]['stock_obj'].time_scales[time_scale]  # Queue

    trainX_s, trainY_s, testX_s, testY_s = None, None, None, None

    for signal_name in signal_names:

        batch = signals_obj.signals[signal_name]['stock_obj'].time_scales[time_scale]  # Queue or QueueObj
    ...
    dl_models_obj.fit(trainX=trainX_s, trainY=trainY_s, testX=testX_s, testY=testY_s, callback=callback,
                      time_scale_index=time_scale_index, stock_monitor=stock_monitor,i=i)
